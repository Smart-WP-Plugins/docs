...
<AccordionGroup>
  ...
  <Accordion title="Will this slow down my store?">
    Unwan creates no custom database tables, runs no extra queries beyond what WooCommerce already performs for customer data, caches its results for the duration of each request, and only loads its scripts and styles on checkout and My Account pages. There's no tracking or external service call of any kind. See [Architecture](/addrvault/developers/architecture).
  </Accordion>
