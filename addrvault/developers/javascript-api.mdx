...
AddrVault's picker is a small, standard-DOM controller shared by Checkout Blocks and classic checkout — there's exactly one implementation to reason about, not two.

## `window.addrVaultAddressPicker`

```js
const picker = window.addrVaultAddressPicker.mount( element, data );
picker.update( nextData );
window.addrVaultAddressPicker.destroy( element );
```
...
The controller expects roughly this shape (matching what `addrvault_checkout_address_options` produces server-side):

```js
{
  addresses: [
    {
      id: 'addrvault_a_1a2b3c4d5e6f7890',
      name: 'Jordan Rivera',
      street: '123 Main St',
  ...
  selection: 'addrvault_a_1a2b3c4d5e6f7890', // a saved ID, or "new"
  summary: { /* the currently selected option, for the collapsed view */ },
  labels: { /* addrvault_checkout_picker_labels keys */ },
  searchThreshold: 4,
  disabled: false,
}
```
...
```js
element.addEventListener( 'addrvault-selection-change', ( event ) => {
...
- No style mutation from JavaScript — all presentation lives in `assets/css/addrvault.css`, documented on the [CSS Variables & Markup](/addrvault/customization/css-and-markup) page.
...
## `window.addrVaultClassicCheckout`

On classic checkout only, AddrVault localizes a private, per-request configuration object consumed by `addrvault-classic-checkout.js` to mount the shared picker into each `.addrvault-checkout__selector` placeholder: checkout options for each enabled type, field keys, the store's base country, the search threshold, and picker labels. This object is checkout-request-specific and isn't intended as a public integration point — filter the data going _into_ it server-side instead, via `addrvault_classic_checkout_picker_data` (see the [Hooks Reference](/addrvault/developers/hooks-reference)).