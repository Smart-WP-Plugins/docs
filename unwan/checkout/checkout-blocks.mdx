...
description: "How Unwan integrates with WooCommerce's block-based checkout"
icon: "cube"
---
...
<Steps>
  <Step title="Capture, before the overwrite">
    When the Store API is about to update the customer object (`woocommerce_store_api_checkout_update_customer_data` / `..._cart_update_customer_data`), Unwan reads and holds the customer's _current_ profile default, if the incoming address differs from it.
  </Step>
