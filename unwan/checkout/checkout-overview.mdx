...
Only a _saved_ address stands in for the fields. If the cart already carries an address that isn't in the book — carried over from an earlier order, or edited away from a saved one — the picker shows it as the current selection but leaves the fields visible and editable underneath, so it can be corrected in place rather than cleared and retyped.

<Note>
  Local pickup is a special case. WooCommerce drops the shipping address from the order and collects only a billing address, while keeping its internal "use shipping as billing" flag set. Unwan suspends its shipping-to-billing synchronisation for as long as collection is active, leaving the billing picker and the billing fields in charge.
</Note>
