...
This walks through one preset end to end: three products, a discount, and a link you can paste into a newsletter. Everything here works on the free version.

## 1. Create the preset

Go to **Cart Presets → Add New Preset**.

Give it a name. The slug is what appears in the link — "Starter Kit" becomes `starter-kit`. Leave the slug empty and it is generated from the name when you save; type your own if you want something shorter. If a slug is already taken a number is added, so a save is never refused over it.
...
Each item carries its own discount, and the preset's total is the sum of its lines. There is no second discount on top.
...
**Custom price** is the useful one for a kit you have advertised at a set price: give each item the price you want it to carry.
...
In the sidebar, **Health checks** asks a series of questions of the preset and answers each one. Before the first save the card is titled **Before your link exists**.
