...
<Info>
  Import and export are Pro features. On the free version the routes are not registered at all — a request 404s like any unknown route. The Settings screen still shows the buttons, locked, so you can see what CartPresets for WooCommerce Pro adds.
</Info>
...
On the destination store, go to **Settings → Data management → Preset data → Import presets** and choose the file. One import takes up to 500 presets; anything beyond that is reported as skipped rather than silently dropped.
...
<Warning>
  ...
  **Variations always travel by ID.** Only the parent product is matched by SKU, so a variation item imported into a different store almost always needs choosing again.
</Warning>
