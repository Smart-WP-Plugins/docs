...
<Info>
  The Presets report is a Pro feature. On the free version there is no menu entry and no report — it is absent rather than switched off.
</Info>
...
The preset listing has a Revenue column in both versions. On free it shows a crown instead of a figure; clicking it explains what Pro would show.
...
Limits count rows in the redemptions table, under **Settings → Usage limits → Count orders with these statuses** (`redemption_statuses`) — so an on-hold order can consume a slot before it counts as a sale on this page.
...
Both fire from CartPresets for WooCommerce Pro only, and the `cartpresets/v1/sales` routes are not registered on the free version — a request 404s like any unknown route. Check the tier before assuming they exist. See the [hooks reference](/cartpresets/developers/hooks-reference#reporting-pro-only).
