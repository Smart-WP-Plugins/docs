...
`cartpresets_clear_cart` decides the outright clear — the whole cart, the customer's own shopping included. It is seeded with the **resolved** answer: the store's **Clear the existing cart**, or the preset's own when its `cart_override` is on. It fires once per link, before the limit check, so the filter and the removal cannot reach different conclusions. The two narrower settings it outranks are read only when it resolves false.

A preset answers all three cart questions or none of them: `cart_override` decides which level is read, and `clear_cart`, `allow_duplicate` and `keep_existing` are its answers when it is on. There is no half-following, so a cart can be predicted from one screen.
...
That list is short for a reason. Narrowing which order statuses consume a slot makes a stored cap count fewer orders than the merchant asked it to, which puts a sold-out run back on sale — so it is gated. Where a refused customer lands cannot do that, whichever way it is set, so the store-wide refusal setting ships free. The per-preset one stays gated, because a downgrade must not silently move a customer a merchant chose to send somewhere.
