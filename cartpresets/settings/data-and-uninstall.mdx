...
| Cart notice and label wording | `delete_settings` | Every store setting (wording included), the stored licence record, each admin's listing preferences, and cached preset data |
...
The only personal data is the redemption record — a user ID or billing email against an order ID. The plugin registers no WordPress personal-data exporter or eraser, so a data request has to be handled in the `cartpresets_redemptions` table directly. Deleting a customer's rows releases any per-customer cap they were holding — which is the correct outcome: with no record of their claim, there is no claim.
