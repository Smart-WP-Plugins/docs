...
A redemption records who claimed a preset, so a per-customer cap can be enforced. For a signed-in customer that is their user ID. For a guest it is their billing email address, lowercased — the only stable way to recognise the same guest twice. No IP address is recorded. A developer can change how customers are identified with `cartpresets_redemption_customer_key`.
...
| The carrier product | `delete_carrier` | The hidden product, if it is still virtual and hidden |
...
WordPress's own tools answer for it. **Tools → Export Personal Data** returns every preset an address claimed, and **Tools → Erase Personal Data** anonymises those claims rather than deleting them, so a spent purchase limit stays spent. Full detail, including why erasure keeps the row: [Privacy and personal data](/cartpresets/settings/privacy).
