...
| Presets already in the cart | `clear_cart` | `global` | Both |
...
`redirect`, `clear_cart` and `unavailable_action` accept `global` (**Use the Settings default**). `clear_cart` otherwise accepts `yes` or `no`; `unavailable_action` accepts `notice` or `redirect`.
