...
A slot is taken when an order enters one of the statuses you nominate, under **Settings → Cart behaviour → Order statuses that use up a purchase limit**.
...
A preset whose total is used up reads as **Sold out**, and what the customer meets is the same choice a [schedule](/cartpresets/building/scheduling) uses: the cart with a notice, or a redirect. A customer turned away by the per-customer cap gets its message and is sent to your store's fallback link.

## Settings reference

| Setting | Level | Key | Default |
| --- | --- | --- | --- |
| Maximum purchases | Preset | `max_total` | Empty (no cap) |
| Maximum per customer | Preset | `max_per` | `0` (no cap) |
