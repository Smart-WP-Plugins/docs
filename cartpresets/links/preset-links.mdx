...
Both halves are yours, under **Settings → Links and fallbacks**:
...
A mistyped slug, a deleted preset, or a rename older than 30 days all reach the same place: your **fallback link**, under **Settings → Links and fallbacks**. Leave it empty and customers go to your shop page.
...
**Append the preset name as a UTM campaign** (**Settings → Links and fallbacks**, off by default) also adds `utm_campaign=<preset slug>` to the destination, so every preset is separable in your analytics without tagging links by hand.
