...
<Note>
  **`not_signed_in` and `role_not_allowed` are store-wide.** The sentence an ineligible visitor reads used to be a field on each preset, which meant one refusal was edited somewhere none of the other nine were. It is here now, beside every other sentence a customer can see — so a wholesale link and a staff link say the same thing, and you write it once.
</Note>
