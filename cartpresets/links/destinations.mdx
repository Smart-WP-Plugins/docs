...
Set store-wide under **Settings → Cart behaviour → After a successful add**, and overridable per preset.
...
| A custom URL | An address you choose, in the **Custom URL** field below the choices | `custom` |

The same four choices, in the same words and each with its own explanation, appear store-wide and on a single preset.
...
Each preset can choose its own destination in the editor's **Cart behaviour** card, under **After the items are added, send the customer to**, or leave it on **Use the Settings default**. That row names the store's current choice, so you can see what following the default means without leaving the page.

## When a link is refused

Nine situations. Eight have their own editable message; the ninth shows the rejection message you wrote on the preset. See [Cart notices and wording](/cartpresets/links/wording).
...
| Not enough room left under the total cap<Pro /> | Fallback URL | `limit_total` |
| Per-customer cap reached<Pro /> | Fallback URL | `limit_customer` |
| The preset has no items | Fallback URL | `no_items` |
| An item cannot be bought, and your rule is "refuse the whole preset" | Fallback URL | `item_blocked` |
| Every item failed at cart time | Fallback URL | `nothing_added` |
| Not eligible<Pro /> | Fallback URL | your rejection message |

### The fallback URL

**Settings → Links and fallbacks → Fallback link**. Leave it empty and refused customers go to your shop page.

### Ended and sold-out presets can redirect instead

A preset whose schedule ended or whose cap filled can send customers elsewhere rather than to the cart. Store-wide, under **Settings → Links and fallbacks → When a preset reaches its limit or its end date**, **Redirect to the fallback link** sends them to your fallback link. On a single preset, **Redirect to a link** takes an address of its own — typically a landing page explaining the offer has closed. Whether they are also told why is the separate **Show a notice that it is unavailable** switch. That is the `unavailable_action` setting, covered under [Scheduling](/cartpresets/building/scheduling)<Pro />.
...
| Fallback link | Store | `not_found_url` | Empty (shop page) |
