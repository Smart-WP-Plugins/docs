...
In the editor, **Presets already in the cart** overrides the store-wide **Clear the existing cart** for one preset: **Use the Settings default** (the default), **Replace it** (stored as `yes`, which empties the cart before this preset is added), or **Add another copy** (`no`, which leaves the cart standing and falls back to the store's other two switches). Key: `clear_cart` on the preset.
